...
-   위 설정은 `HtmlWebpackPlugin`에서 템플릿으로 사용하는 html 파일에도 영향을 준다. `src_study/pages/index.html` 템플릿에서 `<%= require(대상 파일 경로) %>`로 불러오는 `src_study/fragments/headerTag.js` 뿐만 아니라 `HtmlWebpackPlugin`에서 지정한 템플릿인 `src_study/pages/index.html`의 외부 경로를 사용할 때 빌드된 폴더를 또는 로컬 서버의 접근한 도메인 주소를 기준으로 html 파일의 경로 기준으로 한 절대 경로나 상대 경로를 사용하는 것이 아닌 `src_study/pages/index.html` 위치에서의 로컬 파일 경로를 기준으로 경로를 작성해 줘야 한다. `HtmlWebpackPlugin`의 기본 설정은 그대로 유지하여 `HtmlWebpackPlugin`에서 지정한 템플릿의 경로는 그대로 사용하되, `HtmlWebpackPlugin` 템플릿에서 불러오는 html 파일에만 html-loader 로더의 설정이 적용되도록 하자.
-   위 설정이 `HtmlWebpackPlugin`에서 기본적으로 제공하는 템플릿의 경로 설정을 무시하는 이유는 `HtmlWebpackPlugin`에서 지정된 [fallback ejs loader](https://github.com/jantimon/html-webpack-plugin/blob/main/lib/loader.js) lodsh 템플릿 기반의 (` _.template(source, { interpolate: /<%=([\s\S]+?)%>/g, variable: 'data', ...options });`)을 무시하고 html-loader의 설정을 따르기 때문이다.
    > However this also means that in the following example webpack will use the html loader for your template. This will cause html minification and it will also disable the ejs/lodash fallback loader. <sup>[link](https://github.com/jantimon/html-webpack-plugin/blob/main/docs/template-option.md)</sup>
...
-   먼저 `src_study/fragments` 폴더에 `headerTag.html`이란 파일을 만들고 자바스크립트가 아닌 HTML 태그의 일부분을 적어 준다.
...
-   로컬 서버를 켜면 `src_study/fragments/headerTag.html` 파일에 정의한 `html-loader로 불러온 header`를 표시하는 html 태그가 로딩되는 것을 확인할 수 있다.
...
-   `src_study/pages/index.html`의 코드는 body 태그 내의 코드이다.
-   `<img src="assets/img/icon-square-big.svg" />` 부분의 코드를 보면 절대 경로로 이미지 파일을 불러왔다. 하지만 `src_study/fragments/headerTag.html`의 이미지 파일의 경로는 `../assets/img/icon-square-big.svg`으로 상대 주소이다. html-loader로 불러온 html 태그 조각의 외부 파일 경로를 절대주소로 바꾸면 에러가 발생한다.
